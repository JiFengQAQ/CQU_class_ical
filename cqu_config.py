...
def classTimeTranslate(timeStr:str):
    ...
    while True:
        if timeStr.find(',')!=-1: #如果周次有多个范围
            ...
            classList[-1].setdefault("StartWeek",startWeek)
            classList[-1].setdefault("EndWeek",endWeek)
    ...
    for i in range(len(classList)):
        classList[i].setdefault("ClassStartTimeId",classStartTime)
        classList[i].setdefault("ClassEndTimeId",classEndTime)
    return classList
